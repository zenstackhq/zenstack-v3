...
describe('deep nested operations tests', () => {
    ...
    beforeEach(async () => {
        db = await createPolicyTestClient(model);
        rawDb = db.$unuseAll();
    });

    it('works with nested read', async () => {
        await rawDb.m1.create({
            data: {
                myId: '1',
                m2: {
                    create: {
        ...
        r = await db.m3.findUnique({
            where: { id: '3-1' },
            include: { m2: { include: { m1: true } } },
        });
        ...
        expect(r.m2.m1).toBeNull();
    });

    // TODO: should nested create be allowed if it's self-consistent as a whole?
    it.skip('works with nested create', async () => {
        await expect(
            db.m1.create({
                data: {
                    myId: '1',
    ...
    it('works with nested createMany/updateMany/deleteMany', async () => {
        await db.m1.create({
            data: {
                myId: '1',
                m2: {
    ...
    it('works with returning relation when deleting', async () => {
        await db.m1.create({
            data: {
                myId: '1',
                m2: {
