...
it.skip('verifies issue 1574', async () => {
...
model ModelC {
    ...
    // create two users
    const user1 = await db.$unuseAll().user.create({ data: { id: '1' } });
    const user2 = await db.$unuseAll().user.create({ data: { id: '2' } });

    // create two db instances, enhanced for users 1 and 2
    const db1 = db.$setAuth(user1);
    const db2 = db.$setAuth(user2);
    ...
    await db1.modelB.create({ data: { refId: a.id } });
    await db2.modelC.create({ data: { refId: a.id } });
