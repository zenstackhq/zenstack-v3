import { type AstNode, type ConfigurationProvider, type LangiumDocument, type MaybePromise } from 'langium';
...
export class ZModelFormatter extends AbstractFormatter {
    ...
    protected format(node: AstNode): void {
        ...
        if (ast.isDataField(node)) {
            if (ast.isDataModel(node.$container) || ast.isTypeDef(node.$container)) {
        ...
        } else if (ast.isDataFieldAttribute(node)) {
    ...
    private getFieldTypeLength(field: ast.DataField) {
        let length: number;
        ...
        } else if (ast.isDataField(field) && field.type.unsupported) {
