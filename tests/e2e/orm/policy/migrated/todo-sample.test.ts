...
describe('Todo Policy Tests', () => {
    ...
    beforeEach(async () => {
        db = await createPolicyTestClient(schema);
    });
    ...
    it('user', async () => {
        ...
        const user2Db = db.$setAuth({ id: user2.id });
    ...
    it('todo list', async () => {
        ...
        const anonDb = db;
        const emptyUIDDb = db.$setAuth({ id: '' });
        const user1Db = db.$setAuth({ id: user1.id });
        const user2Db = db.$setAuth({ id: user2.id });
        const user3Db = db.$setAuth({ id: user3.id });
    ...
    it('relation query', async () => {
        ...
        const r = await user1Db.space.findFirstOrThrow({
            where: { id: 'space1' },
            include: { lists: true },
        });
        expect(r.lists).toHaveLength(2);

        const r1 = await user2Db.space.findFirstOrThrow({
            where: { id: 'space1' },
            include: { lists: true },
        });
...
async function createSpaceAndUsers(db: ClientContract<SchemaType>) {
    // create users
