...
describe('Regression for issue #1930', () => {
    it('verifies issue 1930', async () => {
    ...
    model OtherContent extends EntityContent {
      data Int
        ...
        const org = await db.$unuseAll().organization.create({ data: {} });
        const article = await db.$unuseAll().article.create({
        ...
        const deletedArticle = await db.$unuseAll().article.create({
            data: { org: { connect: { id: org.id } }, isDeleted: true },
        });
        const content1 = await db.$unuseAll().articleContent.create({
